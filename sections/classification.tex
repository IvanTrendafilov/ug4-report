...
% Features
	...
	- lowercase words
	- use ngrams - yes
	- no prefix or suffix ngrams

\subsection{Training the classifiers}

We have trained both classifiers on data from the 419 corpus.  The training set of the scam variation classifier is 

The training set of the scam variation classifier consists of a 80\% sample of the corpus. $\forall k_{1,2...,22} \in T$, $S=$\{$80\% \times k_{1}, 80\% \times k_{2},..., 80\% \times k_{22}$\}.


The sample is obtained randomly, but is evenly distributed amongst all classes -- i.e., for each class $k$ in the training set $T$, we have sampled 80\% of $k \in T$.

 $N$, $\forall N \in 

The scam variation classifier is trained on a 80% random sample of the corpus 

a 80% random sample of the corpus, distributed evenly amongst all classes

We train both classifiers on data from the 419 corpus. The scam variation classifier is trained on a 80% sample of the corpus. The sample is obtained randomly 

We train both classifiers on data from the 419 corpus. The scam variation classifier is trained o

We train the both models on data from the 419 corpus. 
